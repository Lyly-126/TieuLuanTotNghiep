...
# ====================================
# BƯỚC 2: LẤY DANH SÁCH TỪ ĐÃ CÓ
# ====================================
print("📚 Loading existing words from database...")
cur.execute("SELECT word FROM dictionary")
existing_words: Set[str] = {row[0] for row in cur.fetchall()}
print(f"✅ Found {len(existing_words)} words already in database")

# ====================================
# BƯỚC 3: TẢI DANH SÁCH TỪ CẦN CRAWL
# ====================================
...
print(f"✅ Loaded {len(all_words)} words from list")

# ====================================
# BƯỚC 4: LỌC CHỈ LẤY TỪ CHƯA CÓ
# ====================================
words_to_crawl = [word for word in all_words if word not in existing_words]

print(f"\n🎯 Words to crawl: {len(words_to_crawl)}")
print(f"⏭️  Skipped (already exist): {len(all_words) - len(words_to_crawl)}")
print(f"📊 Coverage: {len(existing_words)/len(all_words)*100:.1f}%")

if not words_to_crawl:
    print("\n🎉 ALL WORDS ALREADY IN DATABASE!")
    cur.close()
    conn.close()
    exit()
...
# ====================================
# BƯỚC 9: CRAWL VỚI MULTITHREADING
# ====================================
start_time = time.time()
MAX_WORKERS = 5

print(f"\n🚀 Starting multithreaded crawl with {MAX_WORKERS} workers...")
print(f"📚 Total words to crawl: {len(words_to_crawl)}")
print("=" * 70)
...
success_rate = success_count / len(words_to_crawl) * 100 if words_to_crawl else 100
...
print(f"⏭️  Skipped: {len(all_words) - len(words_to_crawl)} (already in DB)")
print(f"📊 Total in DB now: ~{len(existing_words) + success_count}")
